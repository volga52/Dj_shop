...
from django.db.models import When, Q, F, Case, IntegerField, DecimalField
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for item in test_orderss:
            print(f'{item.action_order:2}: {item.pk:3}:\
                   {item.product.name:15}: скидка\
                   {abs(item.total_price):6.2f} руб. | \
                   {item.order.updated - item.order.created}')
